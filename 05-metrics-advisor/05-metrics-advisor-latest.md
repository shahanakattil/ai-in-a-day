...
1. You can add a configuration for an individual series or group of series by clicking the **'+'** icon in this window. 
    
    ![](./media/specific%20series.png)

1. Select the **'+'** for **Configuration for specific series (1)**. Specify all **dimension values** **(2)** for series-level configuration to identify a specific series. Leave the **Conditions** as default and select **Save (3)**.

    ![](./media/specifiseries.png)

1. Under the **Metric-level configuration** pane, notice the **Specific series configs** value has changed to **1**.
